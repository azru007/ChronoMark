...
export const CalendarModal: React.FC<CalendarModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/75 backdrop-blur-sm animate-fade-in">
      <div 
        className="relative w-full max-w-lg bg-slate-900 border border-slate-800 rounded-3xl p-5 sm:p-6 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]"
      ...
      >
          ...
          {Array.from({ length: daysInMonth }).map((_, i) => {
            ...
            return (
              <button
                ...
                className={`
                  ...
                  ${isSelected 
                    ...
                    : isToday 
                    ? 'border-2 border-amber-400/80 text-amber-300 bg-amber-500/5' 
                    : hasActivity
              ...
              >
                <span>{dayNum}</span>
                {hasActivity && (
                  <span className={`flex items-center gap-0.5 mt-0.5 ${isSelected ? 'text-slate-900' : 'text-amber-400'}`}>
        ...
        <div className="pt-3 border-t border-slate-800 overflow-y-auto flex-1">
